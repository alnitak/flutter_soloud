  ...
  void process(Inputs &&inputs, int inputSamples, Outputs &&outputs,
    ...
    for (int outputIndex = 0; outputIndex < outputSamples; ++outputIndex) {
      ...
      if (newBlock) {
        ...
        int inputOffset =
            std::round(outputIndex * Sample(inputSamples) / outputSamples);
        ...
        blockProcess.timeFactor =
            didSeek
                ? seekTimeFactor
                : stft.defaultInterval() / std::max<Sample>(1, inputInterval);
        didSeek = false;
      ...
      if (_splitComputation) {
        ...
        processToStep = std::min<size_t>(
            blockProcess.steps, (blockProcess.steps + 0.999f) * processRatio);
      }
