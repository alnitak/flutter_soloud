...
public:
  ...
  float mWet; // Mix between original (dry) and compressed (wet) signal. 0.0 =
              // 100% dry, 1.0 = 100% wet.
  float mThreshold;  // The threshold in dB at which compression starts. Values
                     // lower than the threshold will be compressed.
  float mMakeupGain; // The make-up gain in dB applied to the compressed signal
                     // to compensate for loss in volume due to compression.
  float
      mKneeWidth; // The width in dB of the soft knee where compression smoothly
                  // begins to take effect. A larger value smooths compression.
  float mRatio;   // The compression ratio. The amount by which input exceeding
                // the threshold will be reduced. For example, 4:1 reduces 4 dB
                // of input to 1 dB.
  float mAttackTime;  // The time in ms for the compressor to react to a sudden
                      // increase in input level.
  float mReleaseTime; // The time in ms for the compressor to release the gain
                      // reduction after the input level falls below the
                      // threshold.
