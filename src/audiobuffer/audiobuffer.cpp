...
namespace SoLoud
{
	...
	BufferStreamInstance::BufferStreamInstance(BufferStream *aParent)
	{
		...
		if (mFile)
		{
			...
			else if (mParent->mFiletype == BUFFERSTREAM_OGG)
			{
				int e;
			...
			else if (mParent->mFiletype == BUFFERSTREAM_FLAC)
			{
			...
			else if (mParent->mFiletype == BUFFERSTREAM_MP3)
			{
			...
			else if (mParent->mFiletype == BUFFERSTREAM_PCM)
			{
			}
			else
			{
...
#define MAKEDWORD(a, b, c, d) (((d) << 24) | ((c) << 16) | ((b) << 8) | (a))
		...
		unsigned int aDataLen)
	{
		...
		{
			mBuffer.addData((unsigned char *)aData, aDataLen);
			// mSampleCount += aDataLen / mChannels;
		}
	}
