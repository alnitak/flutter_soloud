...
// DataSource
// ------------------------------------------------------------------
unsigned int MBOggDecoder::DataSource::read(unsigned char *dst,
                                            unsigned int bytes) {
  if (file)
...
MBOggDecoder::MBOggDecoder()
    : mCodecType(CODEC_UNKNOWN), mChannels(0), mSampleRate(0),
      mLengthInSamples(0), mValid(false), mVorbisOpen(false),
      mOpusDecoder(nullptr), mOpusStreamInit(false), mOpusHeaderParsed(false),
      mOpusPacketCount(0), mOpusSkipSamples(0), mOpusTotalOutputSamples(0),
      mOpusTotalSamplesExpected(-1), mOpusMaxFrameSize(0), mOpusPcmReadPos(0),
      mOpusPreSkip(0), mFlacDecoder(nullptr), mFlacPcmReadPos(0),
...
MBOggDecoder::~MBOggDecoder() { close(); }
    ...
    if (pageOut == 1) {
      ...
      if (ogg_stream_init(&os, ogg_page_serialno(&og)) == 0) {
        ...
        if (packetOut == 1) {
          ...
          } else if (op.bytes >= 7 && op.packet[0] == 0x01 &&
                     memcmp(op.packet + 1, "vorbis", 6) == 0) {
            mCodecType = CODEC_VORBIS;
          } else if (op.bytes >= 5 && op.packet[0] == 0x7f &&
                     memcmp(op.packet + 1, "FLAC", 4) == 0) {
...
bool MBOggDecoder::initVorbis() {
  ov_callbacks callbacks = {vorbisReadCb, vorbisSeekCb, vorbisCloseCb,
                            vorbisTellCb};
...
unsigned int MBOggDecoder::readVorbis(float *aBuffer, unsigned int aSamples,
  ...
  while (samplesRead < aSamples) {
    ...
    int bitstream = 0;
    long ret =
        ov_read_float(&mVorbisFile, &pcm, aSamples - samplesRead, &bitstream);
    if (ret <= 0)
      break;

    for (int ch = 0; ch < outChannels; ch++) {
      memcpy(aBuffer + ch * aBufferSize + samplesRead, pcm[ch],
             sizeof(float) * ret);
    }
...
size_t MBOggDecoder::vorbisReadCb(void *ptr, size_t size, size_t nmemb,
                                  void *datasource) {
...
int MBOggDecoder::vorbisSeekCb(void *datasource, ogg_int64_t offset,
                               int whence) {
  DataSource *ds = static_cast<DataSource *>(datasource);
  int absPos = 0;
  switch (whence) {
...
bool MBOggDecoder::initOpus() {
  ...
  while (!mOpusHeaderParsed) {
    if (!feedOpusData()) {
      printf(
          "[MBOggDecoder] initOpus: feedOpusData failed during header parse\n");
      ogg_sync_clear(&mOpusOy);
      return false;
    }
  }
  ...
  printf("[MBOggDecoder] initOpus: success - channels=%d, sampleRate=%d, "
         "length=%u\n",
         mChannels, mSampleRate, mLengthInSamples);
  return true;
}
...
bool MBOggDecoder::decodeOpusPacket(ogg_packet *packet) {
  ...
  int samples =
      opus_decode_float(mOpusDecoder, packet->packet, (opus_int32)packet->bytes,
                        mOpusOutputBuffer.data(), mOpusMaxFrameSize, 0);
  if (samples < 0)
...
unsigned int MBOggDecoder::readOpus(float *aBuffer, unsigned int aSamples,
                                    unsigned int aBufferSize) {
  unsigned int samplesRead = 0;
  int outChannels = mChannels;
  if (outChannels > MAX_CHANNELS)
    ...
    if (avail > 0) {
      ...
      for (int ch = 0; ch < outChannels; ch++) {
        for (size_t i = 0; i < toCopy; i++) {
          aBuffer[ch * aBufferSize + samplesRead + i] =
              mOpusPcmBuffer[mOpusPcmReadPos + i * mChannels + ch];
        }
      }
...
bool MBOggDecoder::initFlac() {
  ...
  // Scan Ogg pages for the last granule position before initializing the
  // decoder.
  unsigned int savedPos = mDataSource.tell();
  ogg_sync_state scanOy;
  ogg_page scanOg;
  ...
  FLAC__StreamDecoderInitStatus status = FLAC__stream_decoder_init_ogg_stream(
      mFlacDecoder, flacReadCb, flacSeekCb, flacTellCb, flacLengthCb, flacEofCb,
      flacWriteCb, flacMetadataCb, flacErrorCb, this);
  ...
  printf("[MBOggDecoder] initFlac: metadata done - channels=%d, sampleRate=%d, "
         "bitsPerSample=%d, length=%u\n",
  ...
  if (mChannels == 0 || mSampleRate == 0) {
    printf("[MBOggDecoder] initFlac: invalid stream info (channels=%d, "
           "sampleRate=%d)\n",
...
unsigned int MBOggDecoder::readFlac(float *aBuffer, unsigned int aSamples,
                                    unsigned int aBufferSize) {
  unsigned int samplesRead = 0;
  int outChannels = mChannels;
  if (outChannels > MAX_CHANNELS)
  ...
  while (samplesRead < aSamples) {
    ...
    if (avail > 0) {
      ...
      for (int ch = 0; ch < outChannels; ch++) {
        for (size_t i = 0; i < toCopy; i++) {
          aBuffer[ch * aBufferSize + samplesRead + i] =
              mFlacPcmBuffer[mFlacPcmReadPos + i * mChannels + ch];
        }
      }
    ...
    FLAC__StreamDecoderState state =
        FLAC__stream_decoder_get_state(mFlacDecoder);
...
MBOggDecoder::flacReadCb(const FLAC__StreamDecoder *decoder,
...
MBOggDecoder::flacSeekCb(const FLAC__StreamDecoder *decoder,
...
MBOggDecoder::flacTellCb(const FLAC__StreamDecoder *decoder,
...
MBOggDecoder::flacLengthCb(const FLAC__StreamDecoder *decoder,
...
FLAC__bool MBOggDecoder::flacEofCb(const FLAC__StreamDecoder *decoder,
                                   void *client_data) {
  (void)decoder;
  MBOggDecoder *self = static_cast<MBOggDecoder *>(client_data);
...
FLAC__StreamDecoderWriteStatus MBOggDecoder::flacWriteCb(
  ...
  // Fallback: populate stream info from frame header if metadata callback
  // missed it
  if (self->mChannels == 0)
  ...
  for (unsigned int i = 0; i < blocksize; i++) {
    for (unsigned int ch = 0; ch < channels; ch++) {
      self->mFlacPcmBuffer.push_back(static_cast<float>(buffer[ch][i]) /
                                     divisor);
    }
  }
...
void MBOggDecoder::flacMetadataCb(const FLAC__StreamDecoder *decoder,
  ...
  if (metadata->type == FLAC__METADATA_TYPE_STREAMINFO) {
    ...
    self->mLengthInSamples =
        (unsigned int)metadata->data.stream_info.total_samples;
  }
}

void MBOggDecoder::flacErrorCb(const FLAC__StreamDecoder *decoder,
                               FLAC__StreamDecoderErrorStatus status,
                               void *client_data) {
  (void)decoder;
  MBOggDecoder *self = static_cast<MBOggDecoder *>(client_data);
  printf("[MBOggDecoder] flacErrorCb: FLAC decoder error: %s (channels=%d, "
         "sampleRate=%d)\n",
         FLAC__StreamDecoderErrorStatusString[status],
         self ? self->mChannels : -1, self ? self->mSampleRate : -1);
}

// ------------------------------------------------------------------
// Public interface
// ------------------------------------------------------------------
bool MBOggDecoder::isValid() const { return mValid; }

MBOggDecoder::CodecType MBOggDecoder::getCodecType() const {
  return mCodecType;
}

int MBOggDecoder::getChannels() const { return mChannels; }

int MBOggDecoder::getSampleRate() const { return mSampleRate; }

unsigned int MBOggDecoder::getLengthInSamples() const {
  return mLengthInSamples;
}

unsigned int MBOggDecoder::read(float *aBuffer, unsigned int aSamples,
                                unsigned int aBufferSize) {
  if (!mValid)
    return 0;
  switch (mCodecType) {
...
bool MBOggDecoder::seek(unsigned int aSample) {
  ...
  case CODEC_FLAC:
    mFlacPcmBuffer.clear();
    mFlacPcmReadPos = 0;
    return FLAC__stream_decoder_seek_absolute(mFlacDecoder, aSample) ? true
                                                                     : false;
  case CODEC_OPUS:
