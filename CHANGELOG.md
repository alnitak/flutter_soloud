...
#### 4.1.5 (XX Xxx 2026)
- **fix: playback and voice errors are no longer silently discarded**. Several native paths ignored failures and reported success, so Dart could get back an unusable `SoundHandle` — or never learn that an operation failed at all:
  - every playback path (`play`, `playClocked`, `playScheduled`, `play3d`, `play3dClocked`, `speechText`, `Bus.playOnEngine`) now checks that the audio output device could actually be (re)started, and rejects the invalid-handle sentinel before treating the result as a handle.
  - that device check is skipped when the voice is requested paused (`play`, `play3d` with `paused: true`, and `Bus.playOnEngine(paused: true)`): a paused voice produces no audio, so you can still create and configure one while an interruption is active. The device is started when you unpause it.
  - `pauseSwitch()`, `setPause()` and `stop()` return an error from C++ now: invalid handles and audio-device failures reach Dart instead of being dropped. The public Dart signatures are unchanged but these methods can now throw. Stopping a sound that ended by itself is still a no-op and does not throw.
  - unpausing that cannot restart the output device leaves the voice paused instead of running it silently.
  - `speechText()` no longer creates any bookkeeping (nor an empty sound entry) when the speech voice cannot be created.
  - `play()`/`play3d()` reaching the maximum number of active voices stays non-blocking (it warns and does not throw), but the unusable handle is no longer registered against the audio source.
  - SoLoud's `play()` now returns the invalid-handle sentinel `0` instead of `UNKNOWN_ERROR` when no voice can be allocated. Handles are encoded as `(voice + 1) | (playIndex << 12)`, so the old error code (7) is itself a legal handle (voice slot 6, play index 0). Once the play index wraps at 0xfffff a live voice owns it, a failure becomes indistinguishable from that voice — `isValidVoiceHandle()` included — and `playClocked`/`playScheduled`/`play3d`/`play3dClocked` would delay, unpause or apply 3D parameters to it. `findFreeVoice_internal()` also no longer falls through to `stopVoice_internal(-1)` when every voice is protected.
  - **`seek()` reports the right error.** SoLoud's own error enum and `PlayerErrors` agree only up to `fileLoadFailed` (3) and then diverge, because `PlayerErrors` inserts `fileAlreadyLoaded` at 4. `seek()` cast one to the other, so SoLoud's `NOT_IMPLEMENTED` (6) — returned when a source cannot seek backwards because it cannot rewind — would surface as `PlayerErrors.outOfMemory` and throw `SoLoudOutOfMemoryException` instead of `SoLoudNotImplementedException`. No built-in audio source can currently reach that code (they all either override `seek` or rewind successfully), so this is not a behavior change today; it removes a trap for any source added later. The same applies to `DLL_NOT_FOUND` and `OUT_OF_MEMORY`, which were shifted by one.
  - **`loadFile()`/`loadMem()` report out-of-memory correctly.** They cast SoLoud's result too, so a `LoadMode.memory` load that runs out of memory slurping the file in (`OUT_OF_MEMORY`, 5) surfaced as `PlayerErrors.dllNotFound` and threw `SoLoudDllNotFoundException` — "The DLL was not found" for a low-memory condition, which is reachable with a large asset on a constrained device. It now throws `SoLoudOutOfMemoryException`.
  - every SoLoud-to-`PlayerErrors` conversion (`loadFile()`, `loadMem()`, `seek()`, `speechText()`, `init()`) maps explicitly through a helper instead of casting, so the mismatch cannot come back.
  - **a failed `loadFile()`/`loadMem()` no longer also raises an uncatchable async error.** Two things caused the same failure to be reported twice: the file-loaded stream listener `throw`ing after it had already delivered the exception through the completer (an exception thrown from a stream listener bypasses the subscription's `onError` and escapes to the zone), and the load result future receiving that error before anything listened to it, which Dart reports as an unhandled async error. The future you `await` is the only channel now.
  - new errors `PlayerErrors.audioDeviceFailedToStart` (34) and `PlayerErrors.failedToStartPlayback` (35), surfaced as `SoLoudAudioDeviceFailedToStartCppException` and `SoLoudFailedToStartPlaybackCppException`.
