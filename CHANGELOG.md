##### 4.1.8 (X Xxx 2026)
- added `joinTwoSources()` to load 2 audio buffers and join them into a single stereo `AudioSource` in memory, converting to mono and resampling to the engine sample rate.
- added experimental render-ahead ring (native only): `SoLoud.init()` accepts `devicePeriodFrames` and `renderAheadFrames`. When `renderAheadFrames` > 0 the engine mixes into an engine-owned ring this many frames ahead of the output device, decoupling the device period from the engine mix quantum, and reactive `play()` / `playScheduled()` calls are mixed retroactively into the not-yet-played portion of the ring — keypress-to-sound latency approaches the small device period even with a large `bufferSize`. Sources that cannot be re-read (released buffer streams, live pull streams, speech) and voices using non-snapshot-able filters degrade gracefully to the previous behavior. New getters: `getPlayheadTime()` (engine time of the sample reaching the device), `getOutputLatency()`, `isRenderAheadEnabled`. Ended-voice callbacks may fire up to `renderAheadFrames` earlier than before when the ring is enabled. Web is unchanged (the ring is ignored there).
