##### 4.1.8 (X Xxx 2026)
> **TL;DR**:
> - **Render-ahead ring (native)**: Ultra-low keypress-to-sound latency via retroactive mixing into a lookahead ring buffer (real-time audio for `playScheduled` and `playClocked` calls which play in the current audio frame, e.g. 10 ms latency).
...
- `changeDevice()` now returns `Future<void>` instead of `void` and runs off the UI thread. Await it to know when the swap finished. Thanks to @Colton127 #508
