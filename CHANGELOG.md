##### 4.1.7 (8 Aug 2026)
- fix: the native engine is now tied to the lifetime of the FlutterEngine that owns it, on Android. A hot restart immediately retires every callback belonging to the discarded isolate — the engine-wide ones and the per-stream buffering/metadata/data-request ones the audio thread calls — and destroying a FlutterEngine (a cached `audio_service` engine, an add-to-app host) disposes the player, output device and scheduler it owned instead of leaving them running with no Dart code able to control them. The blocking part of the teardown runs on a native worker thread, so the platform thread is never held. Activity recreation is unaffected: only FlutterEngine lifecycle events trigger this. Thanks to @Colton127
  - the plugin now declares an Android plugin class. It performs no native work at registration — the native library is loaded lazily, only if one of those transitions actually occurs.
  - supported scope is one active FlutterEngine at a time (including replacing it); two FlutterEngines using the plugin simultaneously is not supported.
  - a mixer output capture started from a worker isolate (`SoLoudIsolate.startMixerOutputStream`) now joins the engine's callback registration instead of standing outside it: it is refused, with a warning in the log, when there is no initialized engine to join — previously it could arm a callback belonging to an isolate that was going away.
- fix: on macOS, the native engine is now torn down when the FlutterEngine that owns it is deallocated, as on iOS. macOS's plugin API has no detach hook at all, so the plugin uses its own deallocation as the signal — reliable because FlutterEngine's `dealloc` releases the references holding it, but reference-count timing rather than a contract: an app that retains the plugin instance simply gets no automatic teardown, as before. Thanks to @Colton127
- fix: on iOS, the native engine is now torn down when the FlutterEngine that owns it is deallocated, instead of leaving a player and output device running with no Dart code able to control them. A stale engine still cannot tear down the engine that replaced it, and the blocking part of the teardown runs on a native worker so the platform thread is never held. Thanks to @Colton127
  - unlike Android, iOS cannot retire callbacks *before* the Dart isolate goes: Flutter's public iOS plugin API has no `onEngineWillDestroy`/`onPreEngineRestart` equivalent, so a window remains where a departed isolate's callbacks are not yet retired. The next `init()` retires stale registrations before claiming. See the README for the exact guarantee.
  - a `deinit()` that runs while the iOS handshake is in flight now wins: the claim is refused rather than landing after the teardown it lost to, which previously left the native engine disposed but still recorded as owned. An explicit refusal from the plugin no longer falls back to claiming directly, and a request whose outcome is unknown is abandoned rather than retried, so a claim can never be taken twice.
  - `SoLoud.init()` still does not require `WidgetsFlutterBinding.ensureInitialized()`. Arming the iOS teardown needs a platform-thread round trip; when messaging is unavailable, initialization proceeds as before and logs that automatic teardown was not armed.
- fix: mixer output capture start/stop are now serialized, and a capture can no longer be started once an engine teardown has begun. `MixerOutput::start()`/`stop()` guarded themselves with a single atomic flag, so two callers could both pass it — a capture stopped from a worker isolate while the engine is being torn down could join the same threads twice — and a capture that started just after teardown was decided could outlive the engine it belonged to, leaving a live notification thread with no player. Starting a capture after `deinit()` (and before the next `init()`) now returns `backendNotInited` instead of silently capturing nothing. Thanks to @Colton127
- fix: a data race on the mixer output capture callback. The notification thread invoked it while another thread could be replacing it — engine teardown with a capture still running, or a capture starting on a second isolate — and reassigning the underlying `std::function` under a concurrent read is undefined behaviour, not just a stale pointer. It is now published and read atomically. Thanks to @Colton127
