...
void main() {
  ...
  test(
    'LinuxAudioBackend is in sync with C++ LinuxAudioBackend in src/enums.h',
    () {
      ...
      expect(
        ...
        isNotNull,
        reason:
            'could not find `typedef enum LinuxAudioBackend` in src/enums.h',
      );
      ...
      for (final match in names) {
        ...
        expect(
          ...
          isNotNull,
          reason:
              'Unexpected C enum value $cName not found in expected mapping',
        );
        expect(
          cValue,
          dartBackend!.value,
